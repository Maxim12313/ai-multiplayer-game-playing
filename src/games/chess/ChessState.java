...
//        List<SearchNode> children = listChildren();
    ...
    public List<SearchNode> listChildren() {
        return findChildren(true);
    }

    private boolean hasChildren() {
        List<SearchNode> children = findChildren(false);
        return ! children.isEmpty();
    }


    private List<SearchNode> findChildren(boolean findAllChildren) {
//        if (children != null)
        ...
        for (int row=0; row<SIZE; row++) {
            for (int col=0; col<SIZE; col++) {
                ...
                if (piece != null && piece.side == getSideToPlay()) {
                    ...
                    if (! findAllChildren && ! children.isEmpty()) return children;
